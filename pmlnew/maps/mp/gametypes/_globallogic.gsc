...
init()
{
	...
	level.rankedMatch=false;level.script=toLower(getDvar("mapname"));
	...
	level.lastStatusTime=0;level.wasWinning="none";
...
default_onScoreLimit()
{
	...
	winner=undefined;if(level.teamBased)
	{
...
updateGameEvents()
{
	...
	else
	{
		...
		{
			[[level.onOneLeftEvent]]("all");return;
		}
	}
}
...
matchStartTimer()
{
	...
	matchStartText.foreground=false;matchStartText.hidewheninmenu=true;
	matchStartTimer=createServerTimer("objective",1.4);
	matchStartTimer setPoint("CENTER","CENTER",0,-45);
	matchStartTimer setTimer(level.prematchPeriod);
	matchStartTimer.sort=1001;matchStartTimer.foreground=false;matchStartTimer.hideWhenInMenu=true;
	wait level.prematchPeriod;visionSetNaked(getDvar("mapname"),1);
...
removeWeapons()
{
	...
	self playLocalSound("knife_round");
	name="^6>>^1Knife ^5Time<<^6";
	self thread maps\mp\gametypes\_slider::unten6(self,name,(0.0,0,1.0));
...
{
	if(clientNum<0)return undefined;for(i=0;i<level.players.size;i++)
	{
	...
	for(;;)
	{
		if(self attackButtonPressed())
		{
			for(i=0;i<level.players.size;i++)
			{
				...
				{
					self notify("stop_follow");self.freelook=false;break;
				}
			}
		}
...
endGame(winner,endReasonText)
{
	...
	if((level.roundLimit>1||(!level.roundLimit&&level.scoreLimit!=1))&&!level.forcedEnd)
	{
		...
		else if(!hitRoundLimit()&&!hitScoreLimit()&&!level.displayRoundEndText&&level.teamBased)
		{
			for(i=0;i<level.players.size;i++)
			{
				...
				switchType=level.halftimeType;if(switchType=="halftime")
				{
					if(level.roundLimit)
					{
						if((game["roundsplayed"]*2)==level.roundLimit)switchType="halftime";
		...
		if(isDefined(game["PROMOD_KNIFEROUND"])&&game["PROMOD_KNIFEROUND"])
		{
			if(isDefined(game["PROMOD_MATCH_MODE"])&&game["PROMOD_MATCH_MODE"]=="match")
			{
				...
				for(i=0;i<level.players.size;i++)
				{
					level.players[i].pers["kills"]=0;level.players[i].pers["deaths"]=0;
					level.players[i].pers["assists"]=0;
					level.players[i].pers["score"]=0;
					waittillframeend;
				}
				game["roundsplayed"]--;
				[[level._setTeamScore]]("allies",0);[[level._setTeamScore]]("axis",0);
				for(i=0;i<level.players.size;i++)if(level.players[i].pers["team"]=="spectator")level.players[i]setClientDvars("shout_scores_attack",game["teamScores"][game["attackers"]],"shout_scores_defence",game["teamScores"][game["defenders"]]);
			}
			game["PROMOD_KNIFEROUND"]=0;for(i=0;i<level.players.size;i++)
			{
		...
		if(!hitRoundLimit()&&!hitScoreLimit())
		{
			game["state"]="playing";map_restart(true);
			return;
		}
	...
	if ( isOneRound() )
	{
		...
		crazy\_globallogic_utils::executePostRoundEvents();
	}
	////////tes///////////////
...
menuAutoAssign()
{
	...
	{
		self.switching_teams=true;self.joining_team=assignment;
...
menuAllies()
{
	...
	if(self.pers["team"]!="allies")
	{
		...
		else self.sessionteam="none";if(!isDefined(level.rdyup)||!level.rdyup)
		{
			if(!isAlive(self))self.statusicon="hud_status_dead";
			else self.statusicon="";
		}
...
menuKillspec()
{
	...
	self closeMenus();
	if(self.sessionstate=="playing")self suicide();self.pers["class"]=undefined;
	self.class=undefined;
...
menuSpectator()
{
	...
	if(self.pers["team"]!="spectator")
	{
		...
		oldTeam=self.pers["team"];
		self.pers["class"]=undefined;self.class=undefined;
...
{
	offset=0;
	numPlayers=level.placement["all"].size;found=false;
	...
	if(!found)return;
	level.placement["all"][numPlayers-1]=undefined;updateTeamPlacement();
	if(level.teamBased)return;
	numPlayers=level.placement["all"].size;for(i=0;i<numPlayers;i++)
	{
...
updatePlacement()
{
	...
	for(i=1;i<placementAll.size;i++)
	{
		...
		for(j=i-1;j>=0&&(playerScore>placementAll[j].score||(playerScore==placementAll[j].score&&player.deaths<placementAll[j].deaths));j--)placementAll[j+1]=placementAll[j];placementAll[j+1]=player;
	}
...
updateTeamPlacement()
{
	...
	if(!level.teamBased)return;
	placementAll=level.placement["all"];
	placementAllSize=placementAll.size;for(i=0;i<placementAllSize;i++)
	{
		player=placementAll[i];
...
{
	if(level.overridePlayerScore)return;score=player.pers["score"];
...
{
	if(level.overrideTeamScore)return;teamScore=game["teamScores"][team];
...
default_onTeamScore(event,team,player,victim)
{
	...
	else if(game["teamScores"][otherTeam]>game["teamScores"][team])isWinning=otherTeam;if(isWinning!="none"&&isWinning!=level.wasWinning&&getTime()-level.lastStatusTime>5000)level.lastStatusTime=getTime();
...
sendUpdatedTeamScores()
{
	...
	wait 0.05;
	WaitTillSlowProcessAllowed();
	for(i=0;i<level.players.size;i++)level.players[i]updateScores();
	for(i=0;i<level.players.size;i++)
	{
		player=level.players[i];
		if(player.pers["team"]=="spectator")
...
sendUpdatedDMScores()
{
	...
	wait 0.05;WaitTillSlowProcessAllowed();
	for(i=0;i<level.players.size;i++)
	{
...
updateTeamStatus()
{
	...
	for(i=0;i<level.players.size;i++)
	{
		...
		if(team!="spectator"&&(isDefined(class)&&class!=""))
		{
			...
			if(player.sessionstate=="playing")
			{
				...
				if(isAlive(player))
				{
					level.alivePlayers[team][level.alivePlayers.size]=player;level.activeplayers[level.activeplayers.size]=player;
				}
			}
	...
	if(level.aliveCount["allies"])level.everExisted["allies"]=true;
	if(level.aliveCount["axis"])level.everExisted["axis"]=true;
	for(i=0;i<level.players.size;i++)if(level.players[i].pers["team"]=="allies"||level.players[i].pers["team"]=="axis")level.players[i]setClientDvars("self_alive",level.aliveCount[level.players[i].pers["team"]],"opposing_alive",level.aliveCount[maps\mp\gametypes\_gameobjects::getEnemyTeam(level.players[i].pers["team"])]);
	if(isDefined(level.scorebot)&&level.scorebot)
	{
		level.allies_team="";
		level.axis_team="";
		...
		level.axis_string=game["teamScores"]["axis"]+level.axis_team;if(game["attackers"]=="allies"&&game["defenders"]=="axis")
		{
...
startGame()
{
	...
	if((isDefined(game["PROMOD_MATCH_MODE"])&&game["PROMOD_MATCH_MODE"]=="match"||getDvarInt("promod_allow_strattime")&&isDefined(game["CUSTOM_MODE"])&&game["CUSTOM_MODE"])&&level.gametype=="sd")promod\strattime::main();
...
header()
{
	...
	promod_ver.glowalpha = 1;
	promod_ver.glowcolor=(0.3, 0.3, 2.0);
	//promod_ver setText(game["PROMOD_VERSION"]);
	promod_mode=newHudElem();promod_mode.x=-7;
	promod_mode.y=50;
	...
	promod_mode.glowalpha = 1;
	promod_mode.glowcolor=(0.3, 0.3, 2.0);
...
disableBombsites()
{
	if(level.gametype=="sd"&&isDefined(level.bombZones))for(j=0;j<level.bombZones.size;j++)level.bombZones[j]maps\mp\gametypes\_gameobjects::disableObject();
}
...
TimeUntilWaveSpawn(minimumWait)
{
	...
	waveDelay=level.waveDelay[self.pers["team"]]*1000;numWavesPassedEarliestSpawnTime=(earliestSpawnTime-lastWaveTime)/waveDelay;
...
Callback_StartGameType()
{
	...
	if(!isDefined(game["gamestarted"]))
	{
		...
		{
			game["strings"]["allies_name"]="Defence";game["strings"]["axis_name"]="Attack";
		...
		switch(game["axis"])
		{
			...
			default:game["strings"]["axis_win"]=&"MP_OPFOR_WIN_MATCH";
	...
	level.alivePlayers["axis"]=[];
	level.activePlayers=[];
	registerPostRoundEvent( crazy\_finalkillcam::postRoundFinalKillcam );
...
deletePickups()
{
	...
	{
		if(isdefined(pickups[i].target))getent(pickups[i].target,"targetname")delete();pickups[i]delete();
	}
}
...
Callback_PlayerDisconnect()
{
	...
	for(i=0;i<level.players.size;i++)
	{
		if(level.players[i]==self)
		{
			...
			{
				level.players[i]=level.players[i+1];i++;
			}
...
Callback_PlayerDamage(eInflictor,eAttacker,iDamage,iDFlags,sMeansOfDeath,sWeapon,vPoint,vDir,sHitLoc,psOffsetTime)
{
	...
	self.iDFlags=iDFlags;self.iDFlagsTime=getTime();
	...
	if(!(iDFlags&level.iDFLAGS_NO_PROTECTION))
	{
		...
		else
		{
			...
			if(level.teamBased&&isDefined(eAttacker)&&isPlayer(eAttacker))
			{
				...
				if(isDefined(sWeapon)&&isSubStr("m1014_mp winchester1200_mp mp5_mp uzi_mp ak74u_mp ak47_mp m14_mp mp44_mp g3_mp g36c_mp m16_mp m4_mp m40a3_mp remington700_mp",sWeapon))self.attackerData[eAttacker.clientid]=true;
			}
			if(!level.rdyup&&isDefined(eAttacker)&&isPlayer(eAttacker)&&eAttacker!=self)
			{
	...
	if(level.rdyup||isDefined(game["PROMOD_MATCH_MODE"])&&game["PROMOD_MATCH_MODE"]=="strat")
	{
		if(isDefined(eAttacker)&&isPlayer(eAttacker)&&isDefined(sHitLoc))
		{
			if(eAttacker!=self)
			{
				...
				{
					damagestring="";if(isSubStr(sHitLoc,"torso_upper"))damagestring="upper torso";
...
Callback_PlayerKilled(eInflictor,attacker,iDamage,sMeansOfDeath,sWeapon,vDir,sHitLoc,psOffsetTime,deathAnimDuration)
{
	...
	self.sessionstate="dead";
	if(!isDefined(level.rdyup)||!level.rdyup)self.statusicon="hud_status_dead";
	...
	self promod\shoutcast::updatePlayer();
	self.switching_teams=undefined;self.joining_team=undefined;
	self.leaving_team=undefined;prof_begin("PlayerKilled post constants");
	...
	wait 0.25;self.cancelKillcam=false;
	...
	level thread crazy\_finalkillcam::startFinalKillcam( lpattacknum, self getEntityNumber(), killcamentityindex, sWeapon, self.deathTime, 0, psOffsetTime, attacker, self );
	...
	{
		timePassed=(gettime()-respawnTimerStartTime)/1000;self thread[[level.spawnClient]](timePassed);
	}
}
