init()
{
	...
	precacheMenu("vote");
	precacheMenu("votekick");
	precacheMenu("clientcmd");
	precacheMenu("menu_vote");
	...
	for(;;)
	{
		...
		switch(response)
		{
			...
			else
			{
				self closeMenu();
				self closeInGameMenu();
				self openMenu(game["menu_vote"]);
			}
			continue;
			case"demo":if(menu=="demo")self.inrecmenu=false;
			continue;
			case"changeteam":self closeMenu();
			self closeInGameMenu();
			self openMenu(game["menu_team"]);
			continue;
			case"shoutcast_setup":if(self.pers["team"]!="spectator")continue;
			self closeMenu();
			self closeInGameMenu();
		...
		switch(menu)
		{
			...
			buf=k[0];for(i=1;i<k.size;i++)buf+=" "+k[i];
			self iprintln(buf);continue;case"team_marinesopfor":case"team_marinesopfor_flipped":switch(response)
			{
				case"allies":self[[level.allies]]();
				break;
				case"axis":self[[level.axis]]();
				break;
				case"autoassign":self[[level.autoassign]]();
				break;
				case"shoutcast":self[[level.spectator]]();
				break;
			}
			continue;case"changeclass_marines_mw":case"changeclass_opfor_mw":if(response=="killspec")
			{
				self[[level.killspec]]();
				continue;
			}
			...
			continue;
			case"quickpromod":maps\mp\gametypes\_quickmessages::quickpromod(response);
	...
	if(isdefined(level.invoting)&&level.invoting==1)
 	{ 
		self closeMenu();
 		self closeInGameMenu(); 
		self openMenu("vote"); 
		self thread crazy\votemap::onDisconnect(); 
		self thread crazy\votemap::updateMenuDisplay(); 
		self setClientDvar( "cg_drawgun", "0" ); 
		self setClientDvar( "hud_ShowWinner", "0" ); 
		self setClientDvar( "hud_voteText", "^2Vote for next map" ); 
		self setClientDvar( "ui_inVote", "1" ); 
	}
	else 
	if(isdefined(level.invoting)&&level.invoting==2) 
	{ 
